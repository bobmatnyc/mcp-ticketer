        ...
        async with get_adapter() as adapter:
            ...
            for field_name, field_data in fields.items():
                ...
                if field_data.get('resource_subtype') == 'enum':
                    assert 'enum_options' in field_data, f"Enum field {field_name} missing 'enum_options'"
    ...
    async def test_priority_update_high(self):
        ...
        async with get_adapter() as adapter:
            ...
            try:
                ...
                assert updated.priority == Priority.HIGH, f"Expected HIGH, got {updated.priority}"
                ...
                priority_field = next(
                    (f for f in raw_task['custom_fields'] if f.get('name', '').lower() == 'priority'),
                    None
                )

                if priority_field:
                    assert priority_field.get('enum_value') is not None, "Priority field has no value"
    ...
    async def test_priority_update_with_enum(self):
        ...
        async with get_adapter() as adapter:
            ...
            try:
                ...
                assert updated.priority in [Priority.HIGH, Priority.MEDIUM], \
...
class TestBug3StateManagement:
        ...
        async with get_adapter() as adapter:
            ...
            try:
                ...
                for state in states_to_test:
                    ...
                    assert updated is not None, f"Update returned None for state {state}"
                    ...
                    if state in [TicketState.DONE, TicketState.CLOSED]:
                        ...
                        assert updated.state != TicketState.DONE or state == TicketState.DONE, \
    ...
    async def test_status_field_preserves_state(self):
        ...
        async with get_adapter() as adapter:
            ...
            try:
                # Set to IN_PROGRESS
                updated = await adapter.update(task.id, {"state": TicketState.IN_PROGRESS})
                ...
                status_field = next(
                    (f for f in raw_task['custom_fields'] if f.get('name', '').lower() == 'status'),
                    None
                )

                if status_field and status_field.get('enum_value'):
                    print(f"Status field value: {status_field['enum_value'].get('name', 'None')}")
                    ...
                    print(f"WARNING: Status field exists but has no value set: {status_field}")
                    # Without Status field value, state will fall back to OPEN (completed=false)
                    assert read_back.state in [TicketState.OPEN, TicketState.IN_PROGRESS], \
                        f"Unexpected state without Status field value: {read_back.state}"
                else:
                    print("WARNING: Status custom field not found - state granularity may be limited")
    ...
    async def test_update_priority_and_state_together(self):
        ...
        async with get_adapter() as adapter:
            ...
            try:
                ...
                updated = await adapter.update(task.id, {
