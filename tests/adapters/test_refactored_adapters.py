...
try:
    ...
    record_test("GitHub adapter has docstring", has_docstring,
...
try:
    ...
    record_test("Jira adapter has docstring", has_docstring,
...
try:
    from mcp_ticketer.adapters.github import GitHubAdapter
    from mcp_ticketer.adapters.github.mappers import build_github_issue_input, task_to_compact_format
    ...
    try:
        ...
        record_test("GitHub adapter has all core methods", all_methods,
    ...
    try:
        ...
        record_test("GitHub mapper function works", mapper_works,
    ...
    try:
        ...
        record_test("GitHub compact format works (BUG DETECTED)", compact_works,
                   f"task_id={has_id}, title={has_title}")
    except AttributeError as e:
        if "'str' object has no attribute 'value'" in str(e):
            record_test("GitHub compact format works (BUG DETECTED)", False,
...
try:
    from mcp_ticketer.adapters.jira import JiraAdapter
    from mcp_ticketer.adapters.jira.mappers import ticket_to_issue_fields, issue_to_ticket
    ...
    try:
        ...
        all_methods = has_create and has_read and has_update and has_list
        record_test("Jira adapter has all core methods", all_methods,
                   f"create={has_create}, read={has_read}, update={has_update}, list={has_list}")
    except Exception as e:
    ...
    try:
        ...
        if has_fields:
            ...
            record_test("Jira mapper function works", mapper_works,
    ...
    try:
        ...
        record_test("Jira type conversion works", is_enum,
                   f"JiraIssueType.TASK = {issue_type}")
    except Exception as e:
...
try:
    ...
    record_test("GitHub adapter uses config dict pattern", signature_valid,
               f"Parameters: {params}")
except Exception as e:
...
try:
    ...
    record_test("Jira adapter uses config dict pattern", signature_valid,
               f"Parameters: {params}")
except Exception as e:
...
try:
    ...
    if hasattr(gh_adapter.GitHubAdapter, 'create_task'):
        ...
        record_test("GitHub adapter has type hints", has_hints or has_return_hint,
                   f"Parameter hints: {has_hints}, Return hint: {has_return_hint}")
    else:
        record_test("GitHub adapter has type hints", False, "create_task method not found")
except Exception as e:
...
try:
    ...
    if hasattr(jira_adapter.JiraAdapter, 'create_task'):
        ...
        record_test("Jira adapter has type hints", has_hints or has_return_hint,
                   f"Parameter hints: {has_hints}, Return hint: {has_return_hint}")
    else:
        record_test("Jira adapter has type hints", False, "create_task method not found")
except Exception as e:
    record_test("Jira adapter has type hints", False, str(e))

# Check for circular imports (if we got here, there are none)
record_test("No circular import dependencies detected", True,
